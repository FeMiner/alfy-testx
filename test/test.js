...
test('entry file', async t => {
  process.chdir('test/fixtures/entry')
  const alfyTest = fn({ entry: 'entry.js' })
...
test('keyword', async t => {
  process.chdir('test/fixtures/keyword')
  const alfyTest = fn({ keyword: 'keyword' })
